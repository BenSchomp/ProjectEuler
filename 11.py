...
86 56  48 35 71 89 7 5 44 44 37 44 60 21 58 51 54 17 58
...
def walk( i, j, iAdj, jAdj ):
  global grid

  # restrict Adj values to -1, 0, 1
  # ensure i, j values are within range based on Adj values
  if( abs(iAdj * jAdj) > 1 ) or \
    ( iAdj == 1 and i > 16 ) or \
    ( iAdj == -1 and i < 3 ) or \
    ( jAdj == 1 and j > 16 ) or \
    ( jAdj == -1 and j < 3 ):
  ...
  return grid[i][j] * \
         grid[i+iAdj][j+jAdj] * \
         grid[i+(iAdj*2)][j+(jAdj*2)] * \
         grid[i+(iAdj*3)][j+(jAdj*3)]

# main
largest = 1
for x in xrange( 0, 20 ):
  for y in xrange( 0, 20 ):
    largest = max( walk( x, y, 1, 0 ), largest )  # right
    largest = max( walk( x, y, 1, -1 ), largest ) # diag: up/right
    largest = max( walk( x, y, 1, 1 ), largest )  # diag: down/right
    largest = max( walk( x, y, 0, 1 ), largest )  # down
